...
class SlideDeck:
    """Utility helpers to render the interactive slide experience."""

    def __init__(self, slide_items: List[Slide]):
        self.slides = slide_items
        self.titles = [f"{idx + 1:02d}. {slide.title}" for idx, slide in enumerate(slide_items)]

    # ------------------------------------------------------------------
    # Session helpers
    def ensure_state(self) -> None:
        """Prime Streamlit session state for navigation widgets."""

        if "slide_index" not in st.session_state:
            st.session_state.slide_index = 0
        if "sidebar_radio" not in st.session_state:
            st.session_state.sidebar_radio = self.titles[0]
        if "show_notes" not in st.session_state:
            st.session_state.show_notes = True

    def sync_sidebar(self, index: int) -> None:
        """Keep all navigation widgets aligned with the active slide."""

        st.session_state.slide_index = index
        st.session_state.sidebar_radio = self.titles[index]

    # ------------------------------------------------------------------
    # Rendering helpers
    def render_sidebar(self) -> None:
        """Render the slide navigator inside the sidebar."""
        ...
        with st.sidebar:
            ...
            self.sync_sidebar(self.titles.index(selected_title))
            st.markdown(
                """
                **디자인 가이드**
                - 슬라이드당 3~5줄 키워드
    ...
    def render_progress(self) -> None:
        """Show the progress bar and slide counter."""

        current_index = st.session_state.slide_index
        progress_value = (current_index + 1) / len(self.slides)
        st.markdown(
            f"<div class='progress-label'><span>{current_index + 1:02d}</span>/"
            f"{len(self.slides):02d} Slides</div>",
            unsafe_allow_html=True,
        )
        dot_markup = "".join(
            f"<div class='progress-dot{' active' if idx == current_index else ''}'></div>"
    ...
    def render_navigation(self) -> None:
        """Render previous/next buttons plus a notes toggle."""

        controls_left, controls_center, controls_right = st.columns([1.5, 2, 1.5])
        with controls_left:
            if st.button(
                "⬅️ 이전",
                ...
                self.sync_sidebar(st.session_state.slide_index - 1)
        ...
        with controls_right:
            if st.button(
                "다음 ➡️",
                use_container_width=True,
                disabled=st.session_state.slide_index == len(self.slides) - 1,
            ):
                self.sync_sidebar(st.session_state.slide_index + 1)
                st.experimental_rerun()

    def render_slide(self) -> None:
        """Paint the active slide inside the faux mobile frame."""

        current_slide = self.slides[st.session_state.slide_index]
        ...
        st.markdown(
            ...
            <div class='slide-content'>
                ...
                {self._render_note(current_slide.note)}
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    def _render_note(self, note: str) -> str:
        """Render speaker notes based on the toggle state."""

        if not st.session_state.show_notes:
            return ""
        return f"<div class='note-box'>💡 <span>Speaker Note</span> {note}</div>"

    def render_footer(self) -> None:
        """Footer legend below the faux frame."""
...
deck.ensure_state()
deck.render_sidebar()
deck.render_progress()
deck.render_navigation()
deck.render_slide()
deck.render_footer()
